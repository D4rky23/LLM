...
async def chat_with_ai(request: ChatRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if request.use_tts and is_tts_available()["any_available"]:
            background_tasks.add_task(generate_tts_background, ai_response, response)
        
        # Generate image if requested
        if request.use_image and is_image_generation_available():
            background_tasks.add_task(generate_image_background, ai_response, response)
...
async def transcribe_audio(file: UploadFile = File(...)):
    ...
    try:
        ...
        temp_path = config.OUTPUT_DIR / f"temp_audio_{uuid.uuid4()}.{file.filename.split('.')[-1]}"
...
async def transcribe_microphone(request: TranscriptionRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Microphone transcription error: {str(e)}")
